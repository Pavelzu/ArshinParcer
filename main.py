...
def removehtmltags(page):
    ...
    page = page.replace ('</pre><div class="json-formatter-container"></div></body></html>',"")
...
def grandresponceparcer(urlind, jsonresp, targetkeys, vriTypekeys, boolkeys):
    ...
    if 'applicable' in jsonresp['result']['vriInfo']:
    ...
    subjsonPoverkaInfo = subjsonPoverkaInfoTranslator(subjsonPoverkaInfo, vriTypekeys) #перевод в нормальные слова Периодическая/первичная
...
def makeFileName(arguments):
    ...
    if filtersDict["verification_year"]:
        fn = fn + filtersDict["verification_year"] + '_'
...
def main():
    ...
    logfilename = makeFileName(sys.argv[1]) + '.log'
    logfilename = logfilename.replace(" ", "_")
    #print(logfilename)

    logging.basicConfig(format=FORMAT, level=level, filename='./logs/' + logfilename)

    with open("theLastLogFile","w") as file:
        file.write(logfilename)
#############
#    logfile = './logs/' + dati + '.log'
...
#################
    ...
    logger.info('Ищем '+ sys.argv[1])

    pid = os.getpid()
    with open("pidlogargs.txt", "a") as file:
        file.write(str(pid) + ' ' + logfilename + ' ' + sys.argv[1]+'\n')
    ...
    targetkeys = { 'mainurl': 'Исходник', 'mitypeNumber': 'Рег номер типа СИ', 'mitypeType': 'Тип СИ', 'mitypeTitle': 'Наименование типа СИ',
                  'manufactureNum': 'Заводской номер СИ', 'regNumber': "Регистрационный номер СИ в реестре ФИФ ОЕИ", 'manufactureYear': 'Год выпуска СИ',
                  'modification': 'Модификация СИ', 'organization': 'Наименование организации-поверителя',
                  'signCipher': 'Условный шифр знака поверки', 'miOwner': 'Владелец СИ', 'vrfDate': 'Дата поверки СИ', 'rankCode' : 'Код разряда эталона в ГПС',
                  'validDate': 'Поверка действительна до', 'docTitle': 'Наименование документа, на основании'
                  'которого выполнена поверка','rankTitle':'Наименование разряда эталона в ГПС/ЛПС' ,'vriType':'Тип поверки', 'certNum':'Номер свидетельства','noticeNum':'Номер извещения', 'signPass':
                  'Знак поверки в паспорте', 'signMi':'Знак поверки на СИ', 'briefIndicator':'Поверка в сокращенном объеме', "SiPrigodno": "СИ пригодно", "additional_info":"Доп сведения",'structure':'Состав СИ, представленного на поверку'}
    ...
    requesttext = makeMainLink(sys.argv[1],rowquantinrequest) + '&start=0'
    ...
    if resquantity == 0:
        ...
        grandinfolist.append("ПУСТО")
        savetoexcel(grandinfolist, sys.argv[1], logger)
        exit(2)
    ...
    if resquantity > int(rowquantinrequest):  # если страниц с результатами больше, чем она
        ...
        for i in range(pagesquantity):
            ...
            while raknagore == 0:
                ...
                ps = getpagesourcecode(requesttext + "&start=" + str(startblock), logger)
                raknagore = get1stdownloadedpagequality(ps,logger)
                #time.sleep(5)
            pageindexes = re.findall(r'"vri_id":"\d-\d*"', ps)
